...
PLAN_RATE = next((c['push']['rate'] for c in PUSHED if c['push'].get('rate_src') == 'product planning rate'), 0.15)
...
lead('What stops it.', f"Five things, each written on the row. (1) The loss stop: the top-of-search price may not pass the point where a top-of-search order costs more than 3× contribution — 3 × contribution (the advertised child’s at today’s price; ${sum(c['push']['contrib'] for c in PUSHED if c['push'].get('contrib')) / max(1, sum(1 for c in PUSHED if c['push'].get('contrib'))):.2f} on average across these rows) × the campaign’s top-of-search conversion rate (its own when it has 15 or more top-of-search clicks in 90 days; under 15, the product’s average across all exact ranking campaigns, {PLAN_RATE:.1%}). A campaign whose own 15+ clicks have produced no top-of-search order takes no step until it converts. Every pushed row carries its cost and loss per order at the new price. (2) The share stop: impression share at 40% or more and not rising after the last step — the requirement is overstated, not the price; it is re-based from actual volume. (3) The reachability check: delivered clicks ÷ impression share is what winning the whole auction would buy; where the requirement is above that — true of most California King rows — the campaign is pushed toward share and graded on share and 7-day rank, not on an unbuyable click count. (4) The envelope (below). (5) The product guard: if weekly TACoS passes 21.7% — the run’s own projection, 1.2× the 18% ceiling of the ranking band — no new step is written and every row holds at its last price until it is back under (operator 2026-09-25). The deal week ran 19.4%.")
