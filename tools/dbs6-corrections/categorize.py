...
W = {k: load(k) for k in ('d90', 'pre_deal30', 'deal8', 'd14', 'd7', 'd3', 'pre0911', 'post0911')}
DAYS = {'d90': 90, 'pre_deal30': 31, 'deal8': 8, 'd14': 14, 'd7': 7, 'd3': 3, 'pre0911': 3, 'post0911': 4}
...
TRACK = {}
_tk = f'{S}/b46/b6_ranks_keywords.json'
...
def target_is(cid, kw, win):
    p = f'{S}/b46/b6_targets_{win}_{cid}.json'
...
def classify(c):
    ...
    contrib = an.get('contrib') or 26.46
    ...
    if focus_in and all_paused:
        ...
        if cat != 'IN_ONTARGET':
            ...
            raised_0915 = any(h['decided'] >= '2026-09-16' and h['field'] == 'placement_multiplier' and h.get('placement') == 'placementTop' and fnum(h['after']) and fnum(h['before']) is not None and fnum(h['after']) > fnum(h['before']) for h in last)
            ...
            if raised_0915 and share_flat and not at_req:
                step /= 2; step_why += f"; halved — impression share did not rise after the 16 Sep raise ({is30:.1f}% → {isd:.1f}%)"
            ...
            stopped = False
            if rate == 0 and p0:
                p1, stopped = p0, True; step_why = f"loss stop — no top-of-search order on its {tos90} top-of-search clicks in 90 days: no step until it converts"
    ...
    restore = []
    for h in last:
        if h['decided'] == '2026-09-11' and h['field'] in ('bid', 'placement_multiplier'):
    ...
    later = [(h['decided'], h['field'], h['placement'], h['before'], h['after']) for h in last if h['decided'] > '2026-09-11']
    return dict(push=push, restore=restore, later=later, cid=cid, name=c['campaign'], short=c['campaign'].replace('DBS6-SP-', ''), obj=c['objective'], focus=c.get('focus'),
