...
late={}
for h in H:
    if h['decided']>='2026-09-21' and h['field'] in ('bid','placement_multiplier','budget'):
...
if stale: D.append(f"**{len(stale)} rows overtaken by live changes on 22 September** — the same lever was already moved after the audit, so these would write over a newer value: seq {', '.join(map(str,stale))} (e.g. {stale_ex[0]}).")
...
if _ec.get('failed'): D.append(f"**{_ec['failed']} engine check{'s' if _ec['failed']>1 else ''} failed and {'are' if _ec['failed']>1 else 'is'} not named** ({_ec.get('passed')} passed). Re-run the check to name {'them' if _ec['failed']>1 else 'it'} before loading.")
