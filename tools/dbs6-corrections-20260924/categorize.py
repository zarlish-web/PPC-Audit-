...
TRACK = {}
_tk = f'{S}/b6_ranks_keywords.json'
...
def target_is(cid, kw, win):
    p = f'{S}/b6_targets_{win}_{cid}.json'
...
def classify(c):
    ...
    contrib = an.get('contrib') or 26.79
    ...
    if focus_in and all_paused:
        ...
        if cat != 'IN_ONTARGET':
            ...
            if raised_0915 and share_flat and not at_req:
                step /= 2; step_why += f"; halved — impression share did not rise after the 16 and 20 Sep raises ({is30:.1f}% → {isd:.1f}%)"
