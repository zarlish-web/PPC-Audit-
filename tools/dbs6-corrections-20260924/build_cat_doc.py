"""Situation-based corrections document for DBS6 run 20260924-981b2555."""
import json, os, re
...
KWSYN = {k: r.get('syntax') for k, r in _dg0.kw_rows('b6', '30d').items()}
SYNM = {r['groupName']: (r.get('market') or {}).get('marketCvr') for r in json.load(open(f'{S}/b46/b6_syntax_30d.json'))['rows']}
OUT = os.environ.get('OUT', '/home/user/PPC-Audit-/docs/DBS6_Corrections_by_Situation_20260924-981b2555.docx')
...
def story(c):
    ...
    b.append(f"Last 10 days (09-15 → 09-24): {R['deal8']['tos']['c']} top-of-search clicks ({c['deal_tos_day']:.1f}/day, against {c['pre_tos_day']:.1f}/day in the 31 days before); last 3 days {R['d3']['t3']} clicks.")
    if c['need_day']:
...
# ================================== document ==================================
t = doc.add_paragraph(); runs(t, 'Corrections to the 24 September run — Decolure Bamboo Sheets 6-Piece', bold=True, size=18, color=NAVY)
para(f"This run sets B6’s focus on four groups — Bamboo|Queen, Bamboo|Full, Bamboo|King and Bamboo|Cooling ({len([c for c in CATS if c['focus_in']])} exact ranking campaigns); Queen and Cooling come in, and California King and the broad Bamboo group go out, against the 21 September run. Focus is taken as the run sets it. B6 has no deal on the calendar (the engine plans none for 09-25 → 10-25), so nothing but money bounds the push: every in-focus exact ranking campaign is pushed on top of search every day from 09-25 until its top-of-search clicks arrive, none held for being thin or for review; no campaign steps past a top-of-search cost per order of 3× contribution; the push runs inside the engine’s own spend plan; and new steps stop if weekly TACoS passes 14.1% — the run’s own projection, from 6.0% today. The target for where clicks land is top of search above 70% and product pages under 20%. Out-of-focus campaigns take no new money and are judged on their own numbers; the ones that convert at the top get a two-week test. B6 keeps its terms during B4’s deal (operator 2026-09-23) — including Bamboo|Cooling, which is now in focus on both products.")
para('Each section gives the rule, what the run did, two or three campaigns where the run’s action is the problem, and the correction. Everything the 21 September B6 document raised has been re-checked on data to 24 September and on the framework as revised on 25 September (the conversion rate that sets each ceiling is now a click-weighted blend from 15 to 49 clicks). Ranks throughout are the tracker’s 7-day median — the engine reads single days. Part D answers Erik’s question for B6 — are we short of top-of-search traffic, or short of CTR/CVR at the top — with the performance-vs-tuning history, competitors, the campaign types that should also be running and the metrics the app should read.')
...
for lab, cs in (('In-focus exact ranking', IN), ('Out-of-focus exact ranking', OUTC), ('All exact ranking', CATS)):
    for w, wl in (('d14', 'last 14 days'), ('deal8', 'last 10 days')):
...
para(f"In focus the job is volume: the {len(IN)} in-focus campaigns take about {sum(c['R']['deal8']['tos']['c'] for c in IN) / 10:.0f} top-of-search clicks a day between them against a requirement of {sum(c['need_day'] or 0 for c in IN):.0f}. Where product pages run over 20% the base comes down in the same write as the push (A-section).")

doc.add_heading('Windows and sources', 2)
para('Amazon’s campaign placement report (Command Center), per campaign: 90 days (06-27 → 09-24) for conversion; the last 14 days (09-11 → 09-24) for where clicks land; the last 7 and 3 days for whether a campaign is still serving and for budget use; the last 10 days (09-15 → 09-24 — B4’s Best Deal was running, B6 had none) for delivery; the 31 days before that (08-15 → 09-14); and 17–19 vs 21–23 September around the 20 September round. Top-of-search impression share is Amazon’s per-target figure over 30 days (08-26 → 09-24) and over the last 10 days. Orders in the last seven days are still filling in (7-day attribution). Stock is the engine’s own read at the audit.')
...
_dsp = sum(c['R']['d7']['spend'] for c in PUSHED) / 7
_proj = sum(c['R']['d7']['spend'] / 7 * ((c['push']['price1'] or 0) / c['push']['price0']) for c in PUSHED if c['push']['price0'])
_full = sum((c['need_day'] or 0) * (c['push']['price1'] or c['push']['price0'] or 0) for c in PUSHED)
lead('The envelope.', f"These campaigns spent ${_dsp:,.0f} a day over the last 7 days; B6’s whole ad account ran $340 a day over 14 days. The push is funded at the engine’s own plan for this run — **$1,072 a day** (the audit workbook’s engine-plan ad spend; $991 after the agent’s holds), **${1072 * 5:,.0f}** for 09-25 → 09-29 (the B6 rule of 23 September: the envelope is the engine plan). At the new prices with clicks unchanged these campaigns would spend about ${_proj:,.0f} a day; the rest is room for the clicks the push buys. Rows keep stepping daily while impression share rises, inside the loss stop and the TACoS guard, until the envelope is spent. Buying every in-focus requirement at the new prices would cost about ${_full:,.0f} a day.")
...
insit(short, f'{len(short)} in-focus campaigns with the right mix, short of their clicks',
      ...
      'Push by the delivery-gap step; raise the budget 30% in the same write wherever the last 7 days used 80% or more of it. On most in-focus rows budget is not what holds clicks back, so the lever is price; the rows that used 80% or more take the budget raise with the step.',
      extra=lambda c: (f" It runs up-and-down bidding and paid {usd(c['R']['deal8']['tos']['s']/c['R']['deal8']['tos']['c'])} per top-of-search click in the last 10 days against a written {usd(c['push']['price0'])} — switch it to fixed, starting near what dynamic actually paid rather than at the written figure plus a step — switching at the written price would roughly halve its bid" if c['push']['ud'] and c['R']['deal8']['tos']['c'] else ''))
leak = by('IN_LEAK')
...
if hb:
    ...
    lead('The rule.', 'A push on a child that runs out before the next arrival rents the position instead of buying it. Switch the ad to the serving child the same day as the push. The run stages these swaps, but the loader skips every swap — do them by hand.')
...
if cs:
    ...
    lead('The rule.', 'Focus stays as this run sets it (Bamboo|Queen, Full, King and Cooling); everything else is out of focus. But an out-of-focus term that already converts at top of search above the bar — 3× the market CVR of its group, on 15 or more top-of-search clicks in 90 days — is tested rather than held: top-of-search price +10% for two weeks (09-25 → 10-08), inside the same loss stop (3× contribution per top-of-search order), on the child it serves (operator 2026-09-23; no budget cut, collapses frozen instead, flagship held — the B4 decisions of 25 September, applied the same way). A campaign that went dark gets its pre-cut base back first.')
    ...
    lead('The read on 10-08.', 'Keep the test price where top-of-search clicks rose and top-of-search conversion stayed above the bar; put it back where they did not. The result is evidence for the next focus decision on California King and the broad Bamboo terms, not a change of focus.')
...
if cs:
    ...
    lead('Correction.', f'Freeze all {len(cs)}; name the cause on each before any price moves again. Where the cause is a stock-out on the child the ad serves, open it as a re-ranking case once stock lands rather than leave it to the taper.')
    import re as _re
    _TR = {r.get('keyword'): {x['date']: x.get('rank') for x in r.get('cells', []) if x.get('state') == 'ranked'} for r in json.load(open(f'{S}/b46/b6_ranks_keywords.json'))['rows']}
    def _m7(k, end):
        from datetime import date as _D, timedelta as _T
        v = sorted(_TR.get(k, {}).get((_D.fromisoformat(end) - _T(days=i)).isoformat()) for i in range(7) if _TR.get(k, {}).get((_D.fromisoformat(end) - _T(days=i)).isoformat()))
        return v[len(v) // 2] if v else None
    _fz = []
    for f in A['context']['rank_freeze']['campaigns']:
        k = _re.search(r'"([^"]+)"', f['why']).group(1); a, b = _m7(k, '2026-08-25'), _m7(k, '2026-09-24')
        if a and b:
            _fz.append([k, f"{f.get('ago30')} → {f.get('now')}", f'{a} → {b}', 'Freeze — lost ' + str(b - a) if b - a > 10 else 'Lift — no loss beyond 10'])
        else:
            _fz.append([k, f"{f.get('ago30')} → {f.get('now')}", '—', 'Freeze on the engine’s read; add to the tracker'])
    lead(f'The engine’s {len(_fz)} rank freezes, read on the 7-day median.', f"The run freezes {len(_fz)} campaigns on single-day reads. On the tracker’s 7-day median (25 August → 24 September), with the threshold at more than 10 positions lost: {sum(1 for x in _fz if x[3].startswith('Freeze —'))} stand, {sum(1 for x in _fz if x[3].startswith('Lift'))} are lifted, {sum(1 for x in _fz if x[2] == '—')} cannot be read.")
    table(['Term', 'Engine read', '7-day median 25 Aug → 24 Sep', 'Verdict'], _fz, widths=[4.4, 3.0, 3.6, 5.8], size=8)
...
rows = []
for date, lab in (('2026-09-16', '16 Sep (12–15 vs 17–19 Sep)'), ('2026-09-20', '20 Sep (17–19 vs 21–23 Sep)')):
...
_g16, _g20 = agg(imp['2026-09-16'], 'Ranking'), agg(imp['2026-09-20'], 'Ranking')
para(f"The 16 September round raised the top-of-search modifier on {_g16['n']} ranking campaigns (spend ${_g16['spa']:.0f} → ${_g16['spb']:.0f} a day, top-of-search clicks {_g16['tosa']:.1f} → {_g16['tosb']:.1f}, orders {_g16['oda']:.1f} → {_g16['odb']:.1f}). The 20 September round touched {_g20['n']} ranking campaigns: spend ${_g20['spa']:.0f} → ${_g20['spb']:.0f} a day, top-of-search clicks {_g20['tosa']:.1f} → {_g20['tosb']:.1f}, orders {_g20['oda']:.1f} → {_g20['odb']:.1f} — the last days of that read are still filling in (7-day attribution). The 21 September document’s read of the 11 September round stands: it halved ranking spend and took top-of-search clicks and orders down with it.")
...
_rows = json.load(open(f'{S}/rows.json')); _wh = sum(1 for r in _rows if r['verdict'] == 'change' and r['withheld'])
for b in [
    '**Read the right window for each question.** Conversion over 90 days (it needs sample); placement mix over the last 14 days; “is it still serving” over the last 3–7 days. Today every price input is 90 days.',
    '**Say when there is no deal.** B6 has no deal on the calendar (09-25 → 10-25 is empty; `engine_read` is null). The push then has no velocity window to lean on; the run should say so and bound the push by money (loss stop, envelope, TACoS guard) rather than by a deal date.',
    '**In focus: push every exact ranking campaign on top of search, every day,** sized by the delivery gap (+30% under 30% delivered, +20% at 30–70%, +10% over 70%), halved where the last raise bought no impression share; no hold for being thin or under review. Money bounds instead of the circular market bound: a loss stop per row (cost per top-of-search order ≤ 3× contribution), a spend envelope, a share stop (≥40% and flat), a reachability check (requirement vs delivered ÷ share) and a product TACoS guard. After the window, rows short of requirement step down to break-even. Out of focus: thin rows hold; rows with a record are judged on their own numbers; converting rows get a bounded test.',
    '**Price every placement on the framework’s blended rate** — the child’s planning rate under 15 clicks, a click-weighted blend from 15 to 49, the campaign’s own from 50 (zero included). The account has no child planning rate; the group rate stands in for it.',
    '**Never write to a paused campaign or one the export does not carry** — see C.3 for the rows in this run.',
    '**Freeze on the 7-day median, not a single day** (Part B reads the engine’s freezes against it).',
    '**Re-read live values before export.** 15 bids and modifiers were moved on 24 September after the audit read them.',
    f"**Carry the gate’s verdict into the export** ({_wh} rows say “change → HOLD” in their own text and export as changes), drop no-op rows, and label each bid row by the keyword it changes.",
    f"**Write predictions the step can deliver** — the book’s rate for the step size, on the window it runs in — not the plan’s requirement. B6’s graded hit rate over 90 days is {A['brief']['sections']['grades']['hit_rate']:.1%} ({A['brief']['sections']['grades']['by_grade']['HIT']} of {A['brief']['sections']['grades']['graded']}); the tuner holds {A['brief']['sections']['tuner']['open']} open records and the run cites none.",
    ...
    '**Check sibling ASINs before writing a bid** — B4 and B6 are both in focus on Bamboo|Cooling this week — and give each shared ranking keyword one owner in the plan.',
    '**Give non-SP builds a way to happen** (a task with an owner and a date), sized to a readable click count.',
]:
    bullet(b)
...
for i, b in enumerate([
    'Pull the rows in section C.3 before loading: withheld, overtaken by the 24 September moves, no-op and duplicate rows, writes on paused campaigns, and the raises on campaigns missing from the export (held until confirmed); drop the budget raises on campaigns not using their budget.',
    'Same day: do the swaps in A-section by hand for the in-focus campaigns on a hero that cannot ship; decide any in-focus campaign whose only keyword is paused; confirm the rank tracker reads the child each ad serves.',
    f"Write the push from live values: {sum(1 for c in PUSHED if c['push']['step'] > 0)} in-focus campaigns stepped on top of search by delivery gap, inside the loss stop, the envelope ($1,072 a day to 09-29) and the 14.1% TACoS guard; base carries the raise past the 900% wall; budget +30% where 80%+ is used.",
    'Every morning to 09-29: top-of-search clicks, impression share, placement, budget use and hours out of budget per in-focus campaign; step again where clicks are short and share rose; halve the step where share was flat; stop at the loss stop, the share stop, the envelope or the TACoS guard; check eligibility where a campaign is still at zero after two steps.',
    'Same day as the push: start the cause checks on the in-focus rank collapses.',
    f"Out of focus: hold the {len(by(['THIN_OUT','THIN_ZERO_OUT']))} thin rows and the flagship flat; freeze the {len(by('COLLAPSE'))} collapses; restore or check the ones that went dark.",
    ...
    'B6 keeps its terms during B4’s deal (operator 2026-09-23), Bamboo|Cooling included: read B6’s impression share on the shared cooling terms daily beside B4’s — if both fall, one listing is taking the other’s auctions.',
    'This week: build the run’s Sponsored Brands, video and display rows at readable budgets.',
    'From 09-30: campaigns at their requirement — probe cost per click down 3–5% a step while click share holds; campaigns short of it — step down to break-even over two writes four days apart (or hold one week where the 7-day rank reached the target range). Grade every write in this list.',
], 1):
    para(f'{i}. {b}')
...
def action(c):
    ...
    if k in ('THIN_PUSH', 'THIN_ZERO_IN', 'SHORT_PRICE'):
        return 'Push TOS modifier: ' + push_price(c)[1] + '; base held; daily read to 09-29'
    if k == 'THIN_TINY':
