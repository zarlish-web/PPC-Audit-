...
def group_table(prod, win='30d'):
    ...
    for g, s in syn.items():
        ...
        ser = gr.get(g, {})
        ranks = {d: at(ser, d) for d in ('2026-06-24', '2026-08-15', '2026-09-14', '2026-09-22')}
...
def kw_table(prod, n_spend=15, plan=None):
    ...
    for k in keys:
        ...
        o = ok.get(k) or {}
        ser = kr.get(k, {})
        ranks = {x: at(ser, x) for x in ('2026-06-24', '2026-08-15', '2026-09-14', '2026-09-22')}
        tc30, tcd = r.get('tos_clicks') or 0, d.get('tos_clicks') or 0
        pre_day = (tc30 - tcd) / 22 if tc30 else 0
        ...
        out.append(dict(kw=k, syntax=r.get('syntax'), sv=r.get('sv'), wsv=wsv, clicks=r.get('clicks'), orders=r.get('orders'), acos=r.get('acos'), spend=r.get('spend'),
                        ...
                        tos_cpc=r.get('tos_cpc'), deal_tos_day=tcd / 8, pre_tos_day=pre_day, is_=t.get('is_'), eff=t.get('eff'), cid=t.get('cid'),
