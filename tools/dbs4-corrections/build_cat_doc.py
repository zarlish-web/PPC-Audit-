...
para('The goal this cycle is to climb the in-focus keywords inside the Best Deal (15–28 September). Every day of the deal counts, so every in-focus exact ranking campaign is pushed on top of search, every day, until its top-of-search clicks arrive — none is held for being thin or for review. The engine’s market limit is off for the deal; what bounds the push is money: no campaign steps past a top-of-search cost per order of 3× contribution (about $75), and the whole push runs inside one spend envelope. The target for where clicks land is top of search above 70% and product pages under 20%. Out-of-focus campaigns take no new money and are judged on their own numbers.')
para('Each section gives the rule, what the run did, two or three campaigns where the run’s action is the problem, and the correction. Ranks throughout are the tracker’s 7-day median — one day’s reading on the head term swings between 16 and 44 in a week. Part D answers Erik’s question for B4 — are we short of top-of-search traffic, or short of CTR/CVR at the top — with the performance-vs-tuning history, competitors, the 6-piece listing in the same auctions, the campaign types that should also be running and the metrics the app should read.')
...
lead('The rule.', 'During the deal every in-focus exact ranking campaign is pushed on the top-of-search modifier, every day, until its required top-of-search clicks arrive. Nothing is held for being thin, for having no clicks yet, or for review — a campaign that is not getting clicks is not in the auction, and only the top-of-search price can put it there.')
lead('How big each day’s step is.', 'Sized by the delivery gap — top-of-search clicks a day in the deal against the requirement: under 30% delivered +30%; 30–70% +20%; over 70% +10%; at or over the requirement, no step. Where the last raise (15 September) did not lift impression share, the step is halved: price that bought no share buys no more by being repeated. Rank distance only breaks ties. A campaign with no click requirement on file takes +20% — a stated departure from the logic document, which would hold it at the ceiling with no premium; these are low-volume terms, and the push doubles as the eligibility check.')
lead('What stops it.', 'Four things, each written on the row. (1) The loss stop: the top-of-search price may not pass the point where a top-of-search order costs more than 3× contribution — 3 × contribution (the advertised child’s; $25.06 on average) × the campaign’s top-of-search conversion rate (its own when it has 15 or more top-of-search clicks in 90 days; under 15, the product’s average across all exact ranking campaigns, 15.5%). Every pushed row carries its cost and loss per order at the new price. Contribution is at list price; the deal price is lower, so the true loss per order in the deal is somewhat higher than written. (2) The share stop: impression share at 40% or more and not rising after the last step — the requirement is overstated, not the price; it is re-based from actual volume. (3) The reachability check: delivered clicks ÷ impression share is what winning the whole auction would buy; where the requirement is above that, the campaign is pushed toward share and graded on share, not on an unbuyable click count. (4) The envelope: the whole deal push runs inside 1.5× today’s daily spend on these campaigns for 09-24 → 09-28; when it is spent, every row holds.')
...
lead('After the deal (from 09-29).', 'The velocity condition lapses with the deal. A campaign that reached its requirement moves to cost-per-click evaluation: probe down 3–5% a step while click share holds. One that did not reach it does not keep stepping: its top-of-search price steps down to break-even (contribution × its top-of-search conversion rate) over two writes four days apart, unless its 7-day rank has moved into the target range, in which case it holds one week at the deal price and is read again. The decision is written on the row.')
...
lead('The envelope.', f"These campaigns spent ${_dsp:,.0f} a day in the deal so far. The push runs inside 1.5× that for 09-24 → 09-28: **${1.5 * _dsp * 5:,.0f}** in total (${1.5 * _dsp:,.0f} a day). At the new prices with clicks unchanged they would spend about ${_proj:,.0f} a day; every click the push adds comes on top, so the envelope — not a price cap — is what ends an over-running push. The loss per order is computed on the full top-of-search price; what Amazon charges is at or below it, so it is an upper bound.")
...
insit(short, f'{len(short)} in-focus campaigns with the right mix, short of their clicks',
      ...
      'Push by the delivery-gap step; raise the budget 30% in the same write wherever the last 7 days used 80% or more of it. On the flagship “Bamboo Sheets” both move: it spent 127% of its budget over the last 7 days, so without the budget raise a higher price only runs it out of money earlier in the day. Read budget use daily; if it still caps out, the next step is budget again.',
      extra=lambda c: (f" It runs up-and-down bidding and paid {usd(c['R']['deal8']['tos']['s']/c['R']['deal8']['tos']['c'])} per top-of-search click in the deal against a written {usd(c['push']['price0'])} — switch it to fixed, starting near what dynamic actually paid rather than at the written figure plus a step — switching at the written price would roughly halve its bid" if c['push']['ud'] and c['R']['deal8']['tos']['c'] else ''))
leak = by('IN_LEAK')
insit(leak, f'{len(leak)} in-focus campaigns with product pages over 20% — cut the base and push top of search in the same write',
      'Product pages over 20% means money landing on the placement that does not move rank. On an in-focus campaign during the deal the fix is one combined write: the base comes down (sized to the leak, never more than 25% where product pages carry orders) and the top-of-search price goes up by the delivery-gap step. The base step is graded on product-page share, the price step on top-of-search clicks, so neither hides the other.',
...
insit(dark, f'{len(dark)} in-focus campaigns that went dark or faded — restore, then push',
      ...
      'Restore the values it last served at where they have not been put back; where the 15 September round already restored them and it still isn’t serving, go straight to the push and check the ad, child and eligibility the same day.',
      extra=lambda c: ((' Already restored: the 10 September round (' + '; '.join(f"{lab} {b}→{a}" for lab, b, a in c['restore'][:3]) + ') was put back on 15 September, so there is nothing to restore — go straight to the push') if (c['restore'] and any(l[0] >= '2026-09-15' for l in c['later'])) else (' Restore first: the 10 September round changed it (' + '; '.join(f"{lab} {b}→{a}" for lab, b, a in c['restore'][:3]) + ')') if c['restore'] else ''))
col = by('IN_COLLAPSE')
...
for b in [
    ...
    '**One rank source, smoothed:** the tracker’s 7-day median for sizing and grading — the engine and Part D disagreed on the head term only because they read different single days.',
...
for i, b in enumerate([
    ...
    'Same day: switch Bamboo Sheets Queen Size to fixed bidding, starting near the ~$12 it was actually paying; confirm which child the tracker reads for the queen terms, then re-point the in-focus campaigns advertising a hero that can’t ship.',
    ...
    'Part D: the 6-piece’s top-of-search modifier to 0% on the terms B4 is ranking, for the deal (a cheap test; read B4’s share the next day). Reset “bamboo sheets queen” and “queen bamboo sheet set” to the group clearing price before stepping.',
